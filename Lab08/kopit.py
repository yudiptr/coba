...
while chain != 'selesai' : # Validating input wheter it is selesai or not
    ...
    chain = input() # looping again until input is selesai
...
def cek_penularan(name): # For cek_penularan, run this
    global command
    info_command = command.split()
    ...
    if len(name):
        ...
        print(f"Maaf, nama {cek_name1} dan {cek_name2} tidak ada dalam rantai penyebaran.")
        ask()
